...
class Robot:
    ...
    def _init_robot_(self, scene, need_topp=False, **kwargs):
        ...
        self.arm_mode = kwargs.get("arm_mode", "dual")
        ...
        self.left_sapien_self_collision_filter = left_embodiment_args.get("sapien_self_collision_filter")
        self.left_ee_pose_source = left_embodiment_args.get("ee_pose_source", "joint")
        ...
        self.right_sapien_self_collision_filter = right_embodiment_args.get("sapien_self_collision_filter")
        self.right_ee_pose_source = right_embodiment_args.get("ee_pose_source", "joint")
        ...
        if self.single_arm_mode:
            ...
            # Compatibility alias only. There is one SAPIEN articulation and
            # single-arm tasks execute exclusively through the left API.
        ...
        self._configure_self_collision_filters()

    def _configure_self_collision_filters(self):
        """Apply opt-in per-link SAPIEN filtering from the embodiment config."""
        seen_entities = set()
        for arm_index, (entity, mode, config_path) in enumerate((
            (self.left_entity, self.left_sapien_self_collision_filter, self.left_curobo_yml_path),
            (self.right_entity, self.right_sapien_self_collision_filter, self.right_curobo_yml_path),
        )):
            if mode is None or id(entity) in seen_entities:
                continue
            seen_entities.add(id(entity))
            if mode != "curobo":
                raise ValueError(f"Unsupported SAPIEN self-collision filter: {mode!r}")

            config_file = config_path if os.path.isabs(config_path) else os.path.join(CONFIGS.ROOT_PATH, config_path)
            with open(config_file, "r", encoding="utf-8") as file:
                config = yaml.safe_load(file)
            ignore_map = config["robot_cfg"]["kinematics"].get("self_collision_ignore", {})
            ...
            if len(ignored_pairs) > 32:
                raise ValueError(
                    f"SAPIEN collision groups support at most 32 ignored pairs, got {len(ignored_pairs)}"
                )

            masks = {}
            for bit, (link_a, link_b) in enumerate(ignored_pairs):
                mask = 1 << bit
                masks[link_a] = masks.get(link_a, 0) | mask
                masks[link_b] = masks.get(link_b, 0) | mask
            ...
            for link_name, mask in masks.items():
                if link_name not in link_by_name:
                    raise ValueError(
                        f"CuRobo collision link {link_name!r} is missing from SAPIEN articulation"
                    )
    ...
    def init_joints(self):
        ...
        if self.left_camera is None:
            ...
            if self.left_camera is None:
                self.left_camera = self.left_entity.find_link_by_name("camera_link")
                if self.left_camera is None:
        ...
        if self.right_camera is None:
            ...
            if self.right_camera is None:
                self.right_camera = self.right_entity.find_link_by_name("camera_link")
                if self.right_camera is None:
    ...
    def _trans_endpose(self, arm_tag=None, is_endpose=False):
        ...
        ee_pose_source = self.left_ee_pose_source if arm_tag == "left" else self.right_ee_pose_source
        if ee_pose_source == "joint":
            ee_pose = ee_joint.global_pose
        elif ee_pose_source == "child_link":
            ee_pose = ee_joint.child_link.get_entity_pose()
        else:
            raise ValueError(f"Unsupported EE pose source: {ee_pose_source!r}")
    ...
    def iter_unique_entities(self):
        """Yield each physical articulation once, ignoring compatibility aliases."""
        seen = set()
    ...
    def _apply_passive_force(self):
        for entity in self.iter_unique_entities():
            self._entity_qf(entity)
