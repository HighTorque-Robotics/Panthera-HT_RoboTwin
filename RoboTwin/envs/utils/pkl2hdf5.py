...
def pkl_files_to_hdf5_and_video(pkl_files, hdf5_path, video_path, metadata=None):
    ...
    observations = data_list["observation"]
    images_to_video(np.array(observations["head_camera"]["rgb"]), out_path=video_path)

    # Keep the existing top-camera video and add one video per wrist camera.
    ...
    for camera_name, suffix in (
        ...
        camera_data = observations.get(camera_name, {})
        frames = camera_data.get("rgb")
        if frames is not None and len(frames) > 0:
            wrist_video_path = os.path.join(video_dir, f"{video_stem}_{suffix}{video_ext}")
            images_to_video(np.array(frames), out_path=wrist_video_path)
