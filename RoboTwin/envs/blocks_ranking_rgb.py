...
class blocks_ranking_rgb(Base_Task):
    ...
    def play_once(self):
        ...
        arm_tag1 = self.pick_and_place_block(self.block1, self.block1_target_pose, block_index=0)
        arm_tag2 = self.pick_and_place_block(self.block2, self.block2_target_pose, block_index=1)
        arm_tag3 = self.pick_and_place_block(self.block3, self.block3_target_pose, block_index=2)
    ...
    def pick_and_place_block(self, block, target_pose=None, block_index=None):
        self.active_block_index = block_index
        block_pose = block.get_pose().p
        arm_tag = ArmTag("left" if self.single_arm_mode or block_pose[0] < 0 else "right")
        self.active_arm_side = str(arm_tag)
        ...
        return "robot" if self.single_arm_mode else str(arm_tag)
    ...
    def check_success(self):
        ...
        if not self.single_arm_mode:
            gripper_open = gripper_open and self.is_right_gripper_open()

        return (np.all(abs(block1_pose[:2] - block2_pose[:2]) < eps)
                and np.all(abs(block2_pose[:2] - block3_pose[:2]) < eps) and block1_pose[0] < block2_pose[0]
                and block2_pose[0] < block3_pose[0] and gripper_open)
