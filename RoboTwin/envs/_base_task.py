...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self.arm_mode = kwags.get("arm_mode", "dual")
        self.single_arm_mode = self.arm_mode == "single"
        self.robot_type = kwags.get("robot_type", kwags.get("embodiment_name", "unknown"))
        ...
        self.setup_scene(
            render_spp=kwags.get("render_spp", 32),
            render_path_depth=kwags.get("render_path_depth", 8),
            render_denoiser=kwags.get("render_denoiser", "oidn"),
        )
        ...
        self.left_joint_path = kwags.get("arm_joint_path", kwags.get("left_joint_path", []))
        ...
        if self.single_arm_mode:
            self.single_arm_setup = self.validate_single_arm_setup()
    ...
    def setup_scene(self, **kwargs):
        ...
        sapien.render.set_ray_tracing_samples_per_pixel(int(kwargs.get("render_spp", 32)))
        sapien.render.set_ray_tracing_path_depth(int(kwargs.get("render_path_depth", 8)))
        sapien.render.set_ray_tracing_denoiser(str(kwargs.get("render_denoiser", "oidn")))
    ...
    def validate_single_arm_setup(self):
        """Fail fast if the single-arm scene regresses into a duplicated setup."""
        unique_entities = list(self.robot.iter_unique_entities())
        if len(unique_entities) != 1 or self.robot.left_entity is not self.robot.right_entity:
            raise RuntimeError("Single-arm mode must contain exactly one robot articulation")

        planner_count = len({id(self.robot.left_planner), id(self.robot.right_planner)})
        if planner_count != 1:
            raise RuntimeError("Single-arm mode must contain exactly one CuRobo planner")

        topp_planner_count = 0
        if self.need_topp:
            topp_planner_count = len({
                id(self.robot.left_mplib_planner),
                id(self.robot.right_mplib_planner),
            })
            if topp_planner_count != 1:
                raise RuntimeError("Single-arm mode must contain exactly one TOPP planner")

        root_pose = unique_entities[0].get_root_pose()
        if not np.isclose(float(root_pose.p[0]), 0.0, atol=1e-9):
            raise RuntimeError(
                f"Single-arm Panthera must be centered at x=0, got x={root_pose.p[0]}"
            )

        wrist_camera_count = 0
        if self.cameras.collect_wrist_camera:
            if not hasattr(self.cameras, "wrist_camera"):
                raise RuntimeError("Single-arm mode must contain one generic wrist_camera")
            wrist_camera_count = 1
        if hasattr(self.cameras, "left_camera") or hasattr(self.cameras, "right_camera"):
            raise RuntimeError("Single-arm mode must not create left/right wrist cameras")

        return {
            "robot_articulation_count": 1,
            "curobo_planner_instance_count": planner_count,
            "topp_planner_instance_count": topp_planner_count,
            "wrist_camera_count": wrist_camera_count,
            "root_pose": root_pose.p.tolist() + root_pose.q.tolist(),
        }

    # =========================================================== Sapien ===========================================================

    def add_physics_step_observer(self, observer):
        """Register an opt-in observer called after every SAPIEN physics step."""
        if not callable(getattr(observer, "on_physics_step", None)):
            raise TypeError("Physics step observer must define on_physics_step(task)")
        self._physics_step_observers.append(observer)

    def clear_physics_step_observers(self):
        self._physics_step_observers.clear()

    def _step_scene(self):
        self.scene.step()
        for observer in tuple(self._physics_step_observers):
            observer.on_physics_step(self)
    ...
    def get_obs(self):
        ...
        if self.data_type.get("endpose", False):
            if self.single_arm_mode:
                pkl_dic["endpose"]["arm"] = self.get_arm_pose("left")
                pkl_dic["endpose"]["gripper"] = self.robot.get_left_gripper_val()
            else:
                norm_gripper_val = [
        ...
        if self.data_type.get("qpos", False):
            ...
            if self.single_arm_mode:
                ...
                pkl_dic["joint_action"]["vector"] = np.asarray(left_jointstate)
            else:
    ...
    def save_traj_data(self, idx):
        ...
        if self.single_arm_mode:
            traj_data = {
                ...
                "arm_mode": "single",
                "arm_joint_path": deepcopy(self.left_joint_path),
            }
        else:
            traj_data = {
    ...
    def set_path_lst(self, args):
        ...
        if self.single_arm_mode:
            self.left_joint_path = args.get("arm_joint_path", [])
    ...
    def delay(self, delay_time, save_freq=None):
        ...
        right_gripper_val = None if self.single_arm_mode else self.robot.get_right_gripper_val()
        for i in range(delay_time):
            if self.single_arm_mode:
                left_result = self.set_gripper(left_pos=left_gripper_val, set_tag="left")
