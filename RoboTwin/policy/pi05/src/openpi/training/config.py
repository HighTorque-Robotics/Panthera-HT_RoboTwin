...
import openpi.policies.panthera_policy as panthera_policy
...
@dataclasses.dataclass(frozen=True)
class LeRobotPantheraDataConfig(DataConfigFactory):
    """Data transforms for RoboTwin's 14-dimensional Panthera dual-arm schema."""

    # RoboTwin stores the next state as an absolute target. OpenPI converts the 12
    # joint dimensions to deltas here; the two gripper dimensions remain absolute.
    use_delta_joint_actions: bool = True
    default_prompt: str | None = None
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        data_transforms = _transforms.Group(
            inputs=[panthera_policy.PantheraInputs()],
            outputs=[panthera_policy.PantheraOutputs()],
        )
        if self.use_delta_joint_actions:
            delta_action_mask = _transforms.make_bool_mask(6, -1, 6, -1)
            data_transforms = data_transforms.push(
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_panthera_full_base",
        model=pi0_config.Pi0Config(pi05=True),
        data=LeRobotPantheraDataConfig(
            repo_id="your_repo_id",
